...
// WASM 定位修正
function showToast(msg) { const t = document.getElementById('toast'); if(!t) return; document.getElementById('toast-msg').innerText = msg; t.classList.remove('translate-y-20', 'opacity-0'); setTimeout(() => t.classList.add('translate-y-20', 'opacity-0'), 3000); }

function escapeHtml(text) {
    return String(text).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
}

function renderTagRankList(tagCounts) {
...
function formatDataDateTime(tradeDate, time) {
...
function getLatestDataDateTime(tradeDate) {
...
function updateSpotQuoteCacheFromQuotes(quotes) {
...
function getSpotQuote(stockId) {
    return spotQuoteCache[stockId] || null;
}

function formatMiniDayCandle(open, high, low, close, size = 'sm') {
...
function formatLabeledMiniCandle(open, high, low, close, label, size = 'sm') {
...
/** 現貨震幅：(最高價 / 最低價 - 1) × 100% */
function formatSpotAmplitude(stockId) {
    const spot = getSpotQuote(stockId);
...
/** 現貨自高點回落：(最高價 - 現價) / 最高價 */
function getSpotPullbackFromHigh(stockId) {
    const spot = getSpotQuote(stockId);
...
function formatSpotPullbackPct(pullback) {
    if (pullback == null || !Number.isFinite(pullback)) return '--';
    return `${(pullback * 100).toFixed(1)}%`;
}

function formatSpotPriceHighCell(stockId) {
    const spot = getSpotQuote(stockId);
...
function buildPullbackUniverse(latestDate, p) {
...
function buildSpotPullbackList() {
...
function renderSpotPullbackSection() {
...
function refreshSpotPullbackSectionOnTick() {
    if (!db) return;
    const p = getParams();
...
function formatFuturesSpotPriceCell(stockId, futuresClose) {
    const spot = getSpotQuote(stockId);
...
function getStockLatestQuote(stockId) {
...
function updateStockAnalysisMeta(stockId) {
...
function patchStockAnalysisLive(stockId) {
    const el = document.getElementById('stock-analysis-meta');
    if (!el || !stockId) return;
...
function formatStockLiveMetric(quote, mode) {
    if (!quote) return '';
    if (mode === 'value') {
...
function patchOverviewStatsLive() {
    const p = getParams();
    const latestDateRes = db.exec(`SELECT MAX(tradeDate) FROM market_data WHERE tradeDate <= ${p.deadline}`);
    if (!latestDateRes.length || !latestDateRes[0].values[0][0]) return;
...
function updatePriceChartLive(stockId) {
...
function showWelcomeWaitingForKey() {
...
function resetDashboardToKeyGate() {
    stopDataRefresh();
    if (db) {
        db.close();
        db = null;
...
function showAccessKeyModal(message) {
...
async function applyAccessKeyAndStartLive(key) {
...
function liveQuoteEndpoint(url) {
...
function formatLiveQuoteError(err, fetchUrl) {
...
function mergeQuotesFuturesClient(quotes, futures) {
    const futuresMap = {};
...
async function fetchQuotesFuturesViaProxy() {
    try {
...
async function fetchWantgooJson(url) {
...
async function ensureFuturesMap() {
...
function upsertQuotesToMemoryDb(quotes, futuresMap) {
...
function getTaipeiMinutesFromMidnight(date = new Date()) {
    const p = getTaipeiDateParts(date);
    return p.hour * 60 + p.minute;
}

function isActiveTradingSession(date = new Date()) {
...
function setLiveQuoteStatus(state, detail, { silent = false } = {}) {
...
function stopLiveQuoteFastPolling() {
    if (liveQuoteTimerId) {
...
function syncLiveQuoteSchedule() {
...
function setCloudDbStatus(state, detail) {
    const el = document.getElementById('db-status');
    if (!el) return;
...
async function fetchMarketDbUint8() {
...
async function applyDatabaseBytes(uint8, { isFirstLoad = false } = {}) {
...
async function refreshCloudDatabase({ silent = false } = {}) {
...
function syncThresholdInputOnModeSwitch(oldMode, newMode) {
    const inp = document.getElementById('param-min-threshold-overview');
    if (!inp) return;
...
function patchFavoritesSectionLive() {
...
// 處置股抓取
async function refreshPunishData() {
...
function normalizeStockCode(id) {
    const s = String(id || '').trim();
    const m = s.match(/^(\d{4})/);
    return m ? m[1] : s;
}

function loadStockTagsMap() {
    stockTagsMap = {};
    if (!db) return;
    try {
        const res = db.exec(`
...
function getTagsForStock(stockId) {
    return stockTagsMap[normalizeStockCode(stockId)] || [];
}

function getPrimaryTag(stockId) {
    const tags = getTagsForStock(stockId);
    return tags.length ? tags[0] : '未標記';
}

function getTagsLabel(stockId) {
    const tags = getTagsForStock(stockId);
    return tags.length ? tags.join('、') : '—';
}

function isFinanceStock(stockId, name) {
...
function findMarketDbEntry(zip) {
...
function updatePriceTrend(stockId) {
    updateStockAnalysisMeta(stockId);
...
function getNewsSearchStartStr(ts) {
    const d = new Date(ts);
...
function getOverviewListLabel() {
    return currentAvgDays <= 1 ? "今日強勢標的" : `近${currentAvgDays}日強勢標的`;
}

function buildPromptGroupRulesBlock(newsSearchStart, newsSearchEnd, stockCount) {
    return `## 搜尋要求
...
function buildOverviewPromptTemplate(analysisDate, listLabel, newsSearchStart, newsSearchEnd, stockLine, stockCount) {
    return `# 強勢標的分組分析
...
function buildSurgePromptTemplate(analysisDate, newsSearchStart, newsSearchEnd, stockLine, stockCount) {
    return `# 量能異常標的分組分析
...
function copyOverviewStockList() {
...
function copyToClipboard(text, toastMsg = '指令已複製！') { const t = document.createElement('textarea'); t.value = text; document.body.appendChild(t); t.select(); document.execCommand('copy'); document.body.removeChild(t); showToast(toastMsg); }
